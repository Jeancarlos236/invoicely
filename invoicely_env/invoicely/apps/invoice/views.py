...
class InvoiceViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        team = self.request.user.teams.first()  # type: ignore
        ...
        serializer.save(created_by=self.request.user, team=team,
                        modified_by=self.request.user, invoice_number=invoice_number, bankaccount=team.bankaccount)
...
def send_reminder(request, invoice_id):
    ...
    text_content = "You have an unpaid invoice. Invoice numberL #" + \
        str(invoice.invoice_number)
    html_content = "You have an unpaid invoice. Invoice numberL #" + \
        str(invoice.invoice_number)
